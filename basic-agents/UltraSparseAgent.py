...
_BUILD_PYLONS = actions.FUNCTIONS.Build_SupplyDepot_screen.id
_BUILD_GATEWAYS = actions.FUNCTIONS.Build_Barracks_screen.id
...
_PROTOSS_PROBE = 84
_PROTOSS_PYLON = 60
_PROTOSS_GATEWAYS = 62
_VESPENE_GEYSER = 342

_PROTOSS_NEXUS = 18
_PROTOSS_PROBE = 45
_PROTOSS_PYLON = 19
_PROTOSS_GATEWAYS = 21
_VESPENE_GEYSER = 341
...
ACTION_BUILD_PYLONS = 'buildpylon'
...
class SparseAgent(base_agent.BaseAgent):
    ...
    def step(self, obs):
        ...
        if obs.first():
            ...
            self.cc_y, self.cc_x = (unit_type == _PROTOSS_NEXUS).nonzero()

        cc_y, cc_x = (unit_type == _PROTOSS_NEXUS).nonzero()
        cc_count = 1 if cc_y.any() else 0

        depot_y, depot_x = (unit_type == _PROTOSS_PYLON).nonzero()
        supply_depot_count = int(round(len(depot_y) / 69))

        barracks_y, barracks_x = (unit_type == _PROTOSS_GATEWAYS).nonzero()
        ...
        if self.move_number == 0:
            ...
            if smart_action == ACTION_BUILD_GATEWAYS or smart_action == ACTION_BUILD_PYLONS:
            ...
            if smart_action == ACTION_BUILD_PYLONS:
                if supply_depot_count < 2 and _BUILD_PYLONS in obs.observation['available_actions']:
                    if self.cc_y.any():
                        ...
                        return actions.FunctionCall(_BUILD_PYLONS, [_NOT_QUEUED, target])

            elif smart_action == ACTION_BUILD_GATEWAYS:
                if barracks_count < 2 and _BUILD_GATEWAYS in obs.observation['available_actions']:
                    if self.cc_y.any():
                        if  barracks_count == 0:
                        ...
                        return actions.FunctionCall(_BUILD_GATEWAYS, [_NOT_QUEUED, target])
                ...
                if len(obs.observation['single_select']) > 0 and obs.observation['single_select'][0][0] == _PROTOSS_PROBE:
                    do_it = False

                if len(obs.observation['multi_select']) > 0 and obs.observation['multi_select'][0][0] == _PROTOSS_PROBE:
                    do_it = False
